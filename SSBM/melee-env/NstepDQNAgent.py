...
class NstepMemory(object):
    ...
    def get_experience(self):
        ...
        for i in range(self.nstep):
            nstep_reward += self.gamma ** i * self.memory[i][2]
        state, action = self.memory[0][0], self.memory[0][1]
        nstep_state, done = self.memory[-1][3], self.memory[-1][4]
        return state, action, nstep_reward, nstep_state, done
...
class NstepDQNAgent(Agent):
    def __init__(self, n_step=1):
        ...
        self.nstep_buffer = NstepMemory(self.n_step, self.GAMMA)
    ...
    def update(self, state, action, reward, next_state, done):
        ...
        if len(self.nstep_buffer) < self.n_step:
            return
        state, action, reward, nstep_state, done = self.nstep_buffer.get_experience()
        
        self.replay_buffer.push(state, action, reward, nstep_state, done)
        if len(self.replay_buffer) < self.BATCH_SIZE:
            return
        states, actions, rewards, nstep_states, dones = map(torch.cat, self.replay_buffer.sample(self.BATCH_SIZE))
        
        Q = self.policy_net(states).gather(1, actions)
        nstepQ = self.target_net(nstep_states).max(1)[0] # 행벡터
        
        target = rewards + (1 - dones) * nstepQ * self.GAMMA ** self.n_step
